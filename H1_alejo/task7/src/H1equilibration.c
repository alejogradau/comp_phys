...
double calc_eq_average(int n_timesteps, double A[], unsigned int i_0){
 ...
 /* Calculate heat capacity for the canonical ensemble (constant NV)
  * formula is taken from H1a.
  */
 double calc_cv_NV(double diff_E, double diff_T)
 {
   double cv = diff_E / diff_T;
   return cv;
 }
