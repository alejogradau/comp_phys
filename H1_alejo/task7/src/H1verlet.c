...
/* Algorithm:
 * Equilibrate system at T
 * Calculate the total energy E of the system
 * call a0, v_0 and q by reference so that their values change
 * This updated values are going to be passed to verlet_thermostat
 *
 * Variables needed:
 * Energy of the system at the equilibration temperatures T+dT and T-dT
 * The REAL equilibration temperatures T+dT and T-dT
 */
void verlet_equilibration(int n_timesteps, double *a0, double Nc, int n_particles,
  double m[n_particles], double v[n_particles][3], double q[n_particles][3],
  double T[n_timesteps], double V[n_timesteps], double E[n_timesteps],
  double dt, unsigned int enable_scaling,
  double temp_inter, double temp_eq, double pressure_eq,
  ...
  double a0_ev[n_timesteps], double time_array[n_timesteps+1])
{
    ...
    double dr = a0/10;
    int minval = a0/(sqrt(2)*dr);
    int maxval = (sqrt(48)*a0)/dr;
    int freqsize = maxval - minval + 1;
    int average[freqsize];
    ...
    for (int i = 1; i < n_timesteps + 1; i++)
    {
        ...
        /* Calculate pair dist. function during production run. */

        if(i >= final_eq_timestep)
        {
           /*
            * Build a frequency_array at timestep i with the counts of each kth
            * distance bin from distances_array
            */
            int numvalues = N*(N-1)/2;
            ...
            {
              int index = int_distances[j] - minval;
              average[index]++;
            }
        }
    }
    /* After each timestep is completed and average[freqsize] contains all the
     * counts per bin along the whole production run, divide over the Number
     * of timesteps in the production run: (n_timesteps - final_eq_timestep)
     */
    for (int j=0; j<freqsize; j++)
    {
      average[j] /= n_timesteps;
    }
  }
}

// Changes temperature alone, does not affect pressure and keeps volume constant
// Receives the a0 from verlet_equilibration and keeps it constant!
void verlet_thermostat(int n_timesteps, double a0, double Nc, int n_particles,
  double m[n_particles], double v[n_particles][3], double q[n_particles][3],
  double T[n_timesteps], double V[n_timesteps], double E[n_timesteps],
  double dt, unsigned int enable_scaling,
