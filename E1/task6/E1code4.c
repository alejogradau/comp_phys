...
int main()
{
  ...
  int n_particles = 3; double kappa = (1.6)*6.25e1;
  ...
  /* Mass of carbon and oxygen atom in asu */
  m[0] = (1 + 1.0/3.0)*1.244e-3;
  m[1] = 1.244e-3;
  m[2] = (1 + 1.0/3.0)*1.244e-3;
  ...
  V[0] += kappa * (pow(q_2[0]-q_1[0],2)
   + pow(q_3[0]-q_2[0],2)) / 2.0;
  ...
  energy_verlet(n_timesteps, n_particles, v,
		...
		q_1, q_2, q_3,
		dt, m, kappa);
    printf("loquequierasenergy\n");

  write_positions_file("csv/displacements.csv", time_array, q_1, q_2, q_3,
		n_timesteps);

/*write_total_displacements_file("abs_total_displacements.csv", time_array,
    			  abs_q_total, n_timesteps);
*/
  write_energy_file("csv/energy.csv", time_array, T, V, E, n_timesteps);

  write_q_file("csv/q1.csv", time_array, q_1, n_timesteps);
  write_q_file("csv/q2.csv", time_array, q_2, n_timesteps);
  write_q_file("csv/q3.csv", time_array, q_3, n_timesteps);
}
