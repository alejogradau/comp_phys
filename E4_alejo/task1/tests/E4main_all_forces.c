 ...
 * Helper functions
 *****************************************************************************/
double calc_acc(double q, double omega, double eta, double v,
                gsl_rng * r3, double sigma2);
...
void BD3(int n_timesteps, double dt,
         ...
         gsl_rng * r1, gsl_rng * r2, gsl_rng * r3,
...
int main()
{
  ...
  // RNG's sigmas
  double sigma1 = 1.0;
  double sigma2 = sqrt(2 * eta * kb * T / mass);
         ...
         double sigma1, double sigma2)
{
    ...
    double a = calc_acc(q, omega, eta, v, r3, sigma2);
    a_arr[0] = a;

    for (int i = 1; i < n_timesteps + 1; i++)
    {
        double gauss1 = gsl_ran_gaussian(r1, sigma1);
        double gauss2 = gsl_ran_gaussian(r2, sigma1);
        ...
        /* a(t+dt) */
        a = calc_acc(q, omega, eta, v, r3, sigma2);
        printf("a_outside: %f\n", a*(1e15));
 ...
 */
double calc_acc(double q, double omega, double eta, double v,
                gsl_rng * r3, double sigma1)
{
    // Initialize variables
    double omega_sq = pow(omega, 2.0);
    double xi = gsl_ran_gaussian(r3, sigma1);
    double a = (-omega_sq * q) - (eta * v) + xi;
